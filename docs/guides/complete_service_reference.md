...
- stateful portfolio valuation normalization uses the same source cash-flow taxonomy as the inspector, so fee-like aliases such as `expense` are normalized into `mgmt_fees` while still producing governance evidence
