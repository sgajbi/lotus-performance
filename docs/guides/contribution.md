...
Only leading portfolio dates before a source-proven position inception (zero opening value plus a
beginning-of-day funding flow) are emitted as zero exposure. A bounded window's first position row
does not by itself prove inception. A missing valuation at or after position inception makes that group return
