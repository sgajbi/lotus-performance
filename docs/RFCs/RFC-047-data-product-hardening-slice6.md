...
`automation/mesh_certification_gate.py` lives. Set `WORKSPACE_ROOT` to the directory holding the
Lotus checkouts first, so the telemetry path resolves into this repository rather than into
`lotus-platform`:

```bash
WORKSPACE_ROOT=/path/to/lotus/checkouts
```
...
python automation/mesh_certification_gate.py --mode advisory --generated-at-utc 2026-05-10T00:00:00Z --telemetry-path "$WORKSPACE_ROOT/lotus-performance/contracts/trust-telemetry" --skip-publication-checks
