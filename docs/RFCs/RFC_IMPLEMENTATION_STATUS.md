...
2.  **RFC 045 — TWR Inspection and Supportability Contract**
    ...
    * **Current state:** **Slices 1-5 are delivered and the source-economics hardening lane has advanced materially.** The durable inspection contract, async runtime path, artifact plumbing, operator/capability wiring, calculation-consistency checks, source-quality/plausibility checks, stateful portfolio-position reconciliation checks, and machine-readable platform-consumption contract now exist. The newest follow-on evidence slices detect both fee-classified and external cash-flow classification loss, fee and external normalization mismatches, duplicate raw source signals, positive fee sign anomalies, conflicting explicit fee or external source totals, external timing-bucket contradictions, non-canonical `cash_flow_type` labels, governed alias labels such as `expense`, unsupported labels such as `dividend`, and unexplained position begin-value carry-forward breaks. Stateful portfolio and position valuation normalization now share the source cash-flow taxonomy so fee-like aliases are preserved in `mgmt_fees` rather than generic cash-flow buckets. A support-facing check inventory document now describes the active inspector findings and artifacts. The next slice is broader timing-anomaly coverage and mandate-aware plausibility.
