...
| RFC-041 | API Orchestrator, Compute Executor, and PostgreSQL Durable State | Proposed | Draft | Partially implemented (requires enhancement) | Current runtime is single-process API + engine path in `main.py`, `app/api/endpoints/performance.py`, `app/api/endpoints/contribution.py`, `app/api/endpoints/returns_series.py`, with filesystem lineage in `app/services/lineage_service.py` | Implement API/executor runtime split, PostgreSQL-backed execution registry, and durable lineage/artifact metadata |
