...
def test_parse_offset_page_token_rejects_invalid_and_negative_tokens():
    with pytest.raises(APIError, match="invalid") as invalid_exc:
    ...
    with pytest.raises(APIError, match="negative") as negative_exc:
        parse_offset_page_token("-1", invalid_detail="invalid token", negative_detail="negative token")
    assert negative_exc.value.status_code == 422
