...
def test_recovery_drill_run_api_does_not_replay_other_operator_correlation(tmp_path, monkeypatch):
    artifact_dir = tmp_path / "artifacts" / "durable-recovery-drill"
    settings = get_settings()
    monkeypatch.setattr(settings, "RECOVERY_DRILL_ARTIFACT_PATH", artifact_dir)
    monkeypatch.setattr(settings, "RECOVERY_DRILL_MANUAL_RUN_COOLDOWN_SECONDS", 300.0)
    ...
    def _fake_run_recovery_drill(**kwargs):
        ...
        output_dir.mkdir(parents=True, exist_ok=True)
        generated_at_utc = datetime.now(UTC).replace(microsecond=0).isoformat().replace("+00:00", "Z")
        evidence_file_name = f"{kwargs['operator_id']}-{kwargs['backup_identifier']}.json"
        payload = {
            "drill_name": "durable_metadata_restore_recovery",
            "generated_at_utc": generated_at_utc,
            "evidence_file_name": evidence_file_name,
...
def test_recovery_drill_run_api_rejects_same_action_when_lease_is_active(tmp_path, monkeypatch):
    ...
    lock_path.write_text(
        (
            '{"action_name":"recovery_drill","operator_id":"ops-user","tenant_id":"tenant-a",'
            f'"governed_target":"backup-123","acquired_at_utc":"{fresh_acquired_at}"'
            "}"
        ),
        encoding="utf-8",
    )
...
def test_recovery_drill_run_api_reclaims_stale_action_lease(tmp_path, monkeypatch):
    artifact_dir = tmp_path / "artifacts" / "durable-recovery-drill"
    settings = get_settings()
    monkeypatch.setattr(settings, "RECOVERY_DRILL_ARTIFACT_PATH", artifact_dir)
    ...
    lock_path = lock_dir / "recovery-drill-ops-user-tenant-a-backup-123.lock"
    lock_path.write_text(
        '{"action_name":"recovery_drill","operator_id":"ops-user","tenant_id":"tenant-a","governed_target":"backup-123","acquired_at_utc":"2026-03-14T00:00:00Z"}',
        encoding="utf-8",
    )
