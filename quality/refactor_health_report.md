...
| Python LOC | 104,454 | 159,241 | measured | LP-CR-1408 diff-based net movement from prior scorecard after the broad shell line count produced an inconsistent historical-method reset |
...
| Largest functions by LOC | unknown | 131 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; `DurableQueueCollector.describe` dropped out of the top-25 in LP-CR-1408 |
...
| Collected tests | 2,035 | 3,123 | measured | `python -m pytest --collect-only -q` |
