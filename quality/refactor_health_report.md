...
| Python LOC | 104,454 | 159,271 | measured | LP-CR-1409 diff-based net movement from prior scorecard after the broad shell line count produced an inconsistent historical-method reset |
...
| Largest functions by LOC | unknown | 130 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; `build_runtime_status_response` moved from `131` to `113` lines in LP-CR-1409 |
...
| Collected tests | 2,035 | 3,124 | measured | `python -m pytest --collect-only -q` |
