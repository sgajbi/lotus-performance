...
| Largest functions by LOC | unknown | 60 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1500 moved `LineageMetadataStore.list_recent_recoveries(...)` out of the top-25 table, and the largest production functions continue to measure `60` lines |
...
| Collected tests | 2,035 | 3,257 | measured | `python -m pytest --collect-only -q` |
