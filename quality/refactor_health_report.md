...
| Python LOC | 104,454 | 143,125 | measured | `rg --files -g '*.py'` plus Python line count on this branch |
...
| Largest functions by LOC | unknown | 73 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1450 moved `calculate_attribution(...)` out of the top-25 table, so the largest production function is now `_build_attribution_supportability_reasons(...)` at `73` lines |
...
| Collected tests | 2,035 | 3,199 | measured | `python -m pytest --collect-only -q` |
