...
| Largest functions by LOC | unknown | 67 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1467 moved `_build_workspace_summary_response(...)` out of the top-25 table, so the largest production functions are now `build_benchmark_exposure_context(...)`, `resolve_contribution_request(...)`, and `_build_flat_period_contribution_result(...)` at `67` lines |
...
| Collected tests | 2,035 | 3,221 | measured | `python -m pytest --collect-only -q` |
