...
| Python LOC | 104,454 | 142,874 | measured | `rg --files -g '*.py'` plus Python line count on this branch |
...
| Largest functions by LOC | unknown | 74 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1447 moved `_build_portfolio_engine_diagnostics(...)` out of the top-25 table, so the largest production functions are now two functions tied at `74` lines |
...
| Collected tests | 2,035 | 3,194 | measured | `python -m pytest --collect-only -q` |
