...
| Python LOC | 104,454 | 142,998 | measured | `rg --files -g '*.py'` plus Python line count on this branch |
...
| Largest functions by LOC | unknown | 73 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1449 moved `_build_workspace_benchmark_and_active_blocks(...)` out of the top-25 table, so the largest production functions are now `calculate_attribution(...)` and `_build_attribution_supportability_reasons(...)` at `73` lines each |
...
| Collected tests | 2,035 | 3,198 | measured | `python -m pytest --collect-only -q` |
