...
| Python LOC | 104,454 | 158,216 | measured | prior measured value plus current Python diff line delta |
...
| Duplicate code hotspots | 0 | 0 | enforced | `quality/duplicate_code_inventory.md`; `make quality-duplicate-code-gate` with `--min-lines 12 --max-groups 0` |
...
| Metric | Baseline | Current | Status | Evidence |
| --- | ---: | ---: | --- | --- |
| Test modules | 228 | 263 | measured | `rg --files tests -g 'test_*.py'` |
| Collected tests | 2,035 | 3,104 | measured | `python -m pytest --collect-only -q` |
