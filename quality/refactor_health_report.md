...
| Metric | Baseline | Current | Status | Evidence |
| --- | ---: | ---: | --- | --- |
| Test modules | 228 | 327 | measured | `rg --files tests -g 'test_*.py'` |
| Collected tests | 2,035 | 4,059 | measured | `python -m pytest --collect-only -q` |
...
| Integration/API/runtime test functions | unknown | 748 | enforced | `quality/test_taxonomy_inventory.md`; `make quality-test-taxonomy-gate` |
| Contract/governance test functions | unknown | 188 | enforced | `quality/test_taxonomy_inventory.md`; `make quality-test-taxonomy-gate` |
| Uncategorized test functions | unknown | 797 | enforced ceiling | `quality/test_taxonomy_inventory.md`; `make quality-test-taxonomy-gate`; Cycle 5 classified async-result and execution-polling access as API/runtime plus applied-currency evidence as analytics, then ratcheted the ceiling to the measured tree. |
...
| Container vulnerability strict gate | unknown | 0 | enforced | `make container-vulnerability-gate` blocks in PR Merge Gate and Main Releasability; zero fixable high/critical findings, with unfixable base-image advisories accepted individually and validated against the same scan by `scripts/container_acceptance_gate.py`, which `make container-vulnerability-gate` runs |
...
| Deployable monitoring dashboard panels | unknown | 12 | enforced | `monitoring/grafana/lotus-performance-operability-dashboard.json`; `make quality-observability-readiness-gate` validates dashboard metric names, labels, links, sensitive-label safety, and alert-backed product supportability coverage |
...
1. Ratcheted `make quality-test-taxonomy-gate` from older accepted floors to an issue #417
   ...
   breadth is `663` API/runtime test functions and `136` contract/governance test functions after
   ...
   Issue #434 added shared retrieval metadata anti-corruption coverage and benchmark exposure API
   degraded-telemetry evidence, raising measured source test functions to `3,492`, API/runtime
   tests to `665`, analytics-domain tests to `1,565`, and uncategorized tests to `968` while
   preserving the existing ceiling.
   Issue #433 added stateful async promotion conflict coverage and classified stateful execution
