...
| Average maintainability index | unknown | 54.97 | measured | `quality/complexity_inventory.md` via `scripts/python_complexity_inventory.py` |
| Largest functions by LOC | unknown | 66 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1472 moved `build_runtime_recoveries_query(...)` out of the top-25 table, so the largest production function remains `_build_flat_period_contribution_result(...)` at `66` lines |
...
| Collected tests | 2,035 | 3,227 | measured | `python -m pytest --collect-only -q` |
