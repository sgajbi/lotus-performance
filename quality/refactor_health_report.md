...
| Largest functions by LOC | unknown | 63 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1483 moved `_load_durable_queue_metric_sources(...)` out of the top-25 table, and the largest production functions now measure `63` lines |
...
| Collected tests | 2,035 | 3,240 | measured | `python -m pytest --collect-only -q` |
