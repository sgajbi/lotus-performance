...
| Largest functions by LOC | unknown | 65 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1474 moved `_calculate_promoted_stateful_benchmark_workflow(...)` out of the top-25 table, while the largest production functions continue to measure `65` lines |
...
| Collected tests | 2,035 | 3,230 | measured | `python -m pytest --collect-only -q` |
