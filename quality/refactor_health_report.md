...
| Python LOC | 104,454 | 158,814 | measured | `rg --files -g '*.py'` plus line count |
...
| Duplicate code hotspots | 0 | 7 | enforced | `quality/duplicate_code_inventory.md`; `make quality-duplicate-code-gate` with `--min-lines 12 --max-groups 7` |
...
| Average maintainability index | unknown | 54.79 | measured | `quality/complexity_inventory.md` via `scripts/python_complexity_inventory.py` |
...
| Collected tests | 2,035 | 3,115 | measured | `python -m pytest --collect-only -q` |
