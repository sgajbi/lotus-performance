...
| Python LOC | 104,454 | 158,676 | measured | `rg --files -g '*.py'` plus line count |
...
| Duplicate code hotspots | 0 | 8 | enforced | `quality/duplicate_code_inventory.md`; `make quality-duplicate-code-gate` with `--min-lines 12 --max-groups 8` |
...
| Average maintainability index | unknown | 54.80 | measured | `quality/complexity_inventory.md` via `scripts/python_complexity_inventory.py` |
...
| Collected tests | 2,035 | 3,114 | measured | `python -m pytest --collect-only -q` |
