...
| Python LOC | 104,454 | 160,456 | measured | `rg --files -g '*.py'` plus PowerShell line count on this branch |
...
| Average maintainability index | unknown | 55.13 | measured | `quality/complexity_inventory.md` via `scripts/python_complexity_inventory.py` |
| Largest functions by LOC | unknown | 113 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1413 added composite TWR capability truth and the next function-size candidate is `_build_analytics_surfaces(...)` |
...
| Collected tests | 2,035 | 3,134 | measured | `python -m pytest --collect-only -q` |
...
| Integration/API/runtime test functions | unknown | 591 | measured | `quality/test_taxonomy_inventory.md` via `scripts/python_test_taxonomy_inventory.py` |
...
| Mapped observability/readiness test functions | unknown | 340 | measured | family-mapped test-function count in `quality/observability_readiness_inventory.md`; counts can overlap across families |
