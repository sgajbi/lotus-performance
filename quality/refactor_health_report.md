...
| Largest functions by LOC | unknown | 68 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1466 moved `StatefulInputService._fetch_position_chunk(...)` from `68` to `65` lines, so the largest production function is now `_build_workspace_summary_response(...)` at `68` lines |
...
| Collected tests | 2,035 | 3,220 | measured | `python -m pytest --collect-only -q` |
