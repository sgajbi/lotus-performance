...
| Python LOC | 104,454 | 161,517 | measured | `rg --files -g '*.py'` plus PowerShell line count on this branch |
...
| Largest functions by LOC | unknown | 89 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1427 moved `_calculate_returns_series(...)` from `90` to `49` lines and out of the top-25 table by isolating returns-series execution result assembly; the largest production function is now `_build_artifacts(...)` at `89` lines |
...
| Collected tests | 2,035 | 3,149 | measured | `python -m pytest --collect-only -q` |
