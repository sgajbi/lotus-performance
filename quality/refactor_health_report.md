...
| Python LOC | 104,454 | 160,916 | measured | `rg --files -g '*.py'` plus PowerShell line count on this branch |
...
| Largest functions by LOC | unknown | 93 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1420 moved `resolve_attribution_request(...)` from `94` to `74` lines by isolating attribution stage-detail projection; the largest production functions are now `_build_analytics_surfaces(...)`, `build_stateful_benchmark_input(...)`, and `calculate_twr_response(...)` at `93` lines each |
...
| Collected tests | 2,035 | 3,141 | measured | `python -m pytest --collect-only -q` |
