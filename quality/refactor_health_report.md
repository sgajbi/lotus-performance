...
| --- | ---: | ---: | --- | --- |
| Python files | 480 | 553 | measured | `rg --files -g '*.py'` |
| Python package markers | 18 | 18 | measured | recursive `__init__.py` count |
| Python LOC | 104,454 | 159,033 | measured | `rg --files -g '*.py'` plus line count |
...
| Duplicate code hotspots | 0 | 2 | enforced | `quality/duplicate_code_inventory.md`; `make quality-duplicate-code-gate` with `--min-lines 12 --max-groups 2`; duplicated LOC reduced from `96` to `54` in LP-CR-1405 |
...
| Collected tests | 2,035 | 3,120 | measured | `python -m pytest --collect-only -q` |
