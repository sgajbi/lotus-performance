...
| Largest functions by LOC | unknown | 69 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1462 moved `_process_pending_jobs(...)` out of the top-25 table, so the largest production functions now measure `69` lines |
...
| Collected tests | 2,035 | 3,216 | measured | `python -m pytest --collect-only -q` |
