...
| Python LOC | 104,454 | 161,428 | measured | `rg --files -g '*.py'` plus PowerShell line count on this branch |
...
| Largest functions by LOC | unknown | 90 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1426 moved `_build_hierarchy_period_contribution_result(...)` from `90` lines out of the top-25 table by isolating hierarchy contribution position and hierarchy projection assembly; the largest production function is now `_calculate_returns_series(...)` at `90` lines |
...
| Collected tests | 2,035 | 3,148 | measured | `python -m pytest --collect-only -q` |
