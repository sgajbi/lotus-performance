...
| Largest functions by LOC | unknown | 65 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1478 moved `build_runtime_retention_history_snapshot(...)` from `65` to `50` lines and out of the top-25 table, while the largest production function is now `StatefulInputService._fetch_position_chunk(...)` at `65` lines |
...
| Collected tests | 2,035 | 3,234 | measured | `python -m pytest --collect-only -q` |
