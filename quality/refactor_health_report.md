...
| Largest functions by LOC | unknown | 66 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1470 moved `_build_flat_period_contribution_result(...)` from `67` to `66` lines and `_build_hierarchy_period_contribution_result(...)` from `64` to `63` lines, so the largest production functions now measure `66` lines |
...
| Collected tests | 2,035 | 3,224 | measured | `python -m pytest --collect-only -q` |
