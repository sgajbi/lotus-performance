...
| Average maintainability index | unknown | 55.14 | measured | `quality/complexity_inventory.md` via `scripts/python_complexity_inventory.py` |
| Largest functions by LOC | unknown | 66 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1471 moved `build_runtime_work_item_snapshot(...)` out of the top-25 table, so the largest production function remains `_build_flat_period_contribution_result(...)` at `66` lines |
...
| Metric | Baseline | Current | Status | Evidence |
| --- | ---: | ---: | --- | --- |
| Test modules | 228 | 273 | measured | `rg --files tests -g 'test_*.py'` |
| Collected tests | 2,035 | 3,225 | measured | `python -m pytest --collect-only -q` |
...
| Integration/API/runtime test functions | unknown | 598 | measured | `quality/test_taxonomy_inventory.md` via `scripts/python_test_taxonomy_inventory.py` |
