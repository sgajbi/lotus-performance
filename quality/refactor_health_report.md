...
| Largest functions by LOC | unknown | 68 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1465 moved `_calculate_promoted_stateful_returns_series(...)` out of the top-25 table, so the largest production functions now measure `68` lines |
...
| Collected tests | 2,035 | 3,219 | measured | `python -m pytest --collect-only -q` |
