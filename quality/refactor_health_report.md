...
| Average maintainability index | unknown | 54.95 | measured | `quality/complexity_inventory.md` via `scripts/python_complexity_inventory.py` |
| Largest functions by LOC | unknown | 64 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1479 moved `StatefulInputService._fetch_position_chunk(...)` out of the top-25 table, and the largest production functions are now `resolve_attribution_request(...)` and `LineageMetadataStore._build_pending_payload_stats_statement(...)` at `64` lines |
...
| Collected tests | 2,035 | 3,235 | measured | `python -m pytest --collect-only -q` |
...
| Integration/API/runtime test functions | unknown | 599 | measured | `quality/test_taxonomy_inventory.md` via `scripts/python_test_taxonomy_inventory.py` |
