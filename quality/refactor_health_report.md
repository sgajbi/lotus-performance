...
| Average maintainability index | unknown | 54.94 | measured | `quality/complexity_inventory.md` via `scripts/python_complexity_inventory.py` |
| Largest functions by LOC | unknown | 63 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1482 moved `_SourceEconomicsSampleCollector._record_taxonomy_samples(...)` out of the top-25 table, and the largest production functions now measure `63` lines |
...
| Collected tests | 2,035 | 3,239 | measured | `python -m pytest --collect-only -q` |
