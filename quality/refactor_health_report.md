...
| Average maintainability index | unknown | 54.93 | measured | `quality/complexity_inventory.md` via `scripts/python_complexity_inventory.py` |
| Largest functions by LOC | unknown | 62 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1487 moved `resolve_contribution_request(...)` out of the top-25 table, and the largest production functions now measure `62` lines |
...
| Collected tests | 2,035 | 3,244 | measured | `python -m pytest --collect-only -q` |
