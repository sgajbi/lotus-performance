...
| Largest functions by LOC | unknown | 67 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1469 moved `resolve_contribution_request(...)` from `67` to `62` lines, so the largest production function is now `_build_flat_period_contribution_result(...)` at `67` lines |
...
| Collected tests | 2,035 | 3,223 | measured | `python -m pytest --collect-only -q` |
