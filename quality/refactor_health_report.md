...
| Largest functions by LOC | unknown | 80 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1435 moved `_build_contribution_response(...)` out of the top-25 table and moved `calculate_contribution(...)` from `80` to `74` lines by isolating response evidence assembly and execution lineage handoff; the largest production function is now `LineageMetadataStore._build_inspection_query_statements(...)` at `80` lines |
...
| Collected tests | 2,035 | 3,161 | measured | `python -m pytest --collect-only -q` |
