...
| Python LOC | 104,454 | 160,961 | measured | `rg --files -g '*.py'` plus PowerShell line count on this branch |
...
| Largest functions by LOC | unknown | 93 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1421 moved `_build_analytics_surfaces(...)` from `93` to `52` lines by isolating portfolio/workspace analytics surface projection; the largest production functions are now `build_stateful_benchmark_input(...)` and `calculate_twr_response(...)` at `93` lines each |
...
| Collected tests | 2,035 | 3,142 | measured | `python -m pytest --collect-only -q` |
