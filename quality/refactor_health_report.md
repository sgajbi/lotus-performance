...
| Average maintainability index | unknown | 54.92 | measured | `quality/complexity_inventory.md` via `scripts/python_complexity_inventory.py` |
| Largest functions by LOC | unknown | 61 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1496 moved `StatefulInputService._fetch_portfolio_chunk(...)` out of the top-25 table, and the largest production functions continue to measure `61` lines |
...
| Collected tests | 2,035 | 3,252 | measured | `python -m pytest --collect-only -q` |
