...
| Python LOC | 104,454 | 142,971 | measured | `rg --files -g '*.py'` plus Python line count on this branch |
...
| Largest functions by LOC | unknown | 74 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1448 moved `calculate_contribution(...)` out of the top-25 table, so the largest production function is now `_build_workspace_benchmark_and_active_blocks(...)` at `74` lines |
...
| Collected tests | 2,035 | 3,197 | measured | `python -m pytest --collect-only -q` |
...
| Integration/API/runtime test functions | unknown | 597 | measured | `quality/test_taxonomy_inventory.md` via `scripts/python_test_taxonomy_inventory.py` |
