...
| Largest functions by LOC | unknown | 64 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1480 moved `resolve_attribution_request(...)` out of the top-25 table, and the largest production function is now `LineageMetadataStore._build_pending_payload_stats_statement(...)` at `64` lines |
...
| Collected tests | 2,035 | 3,237 | measured | `python -m pytest --collect-only -q` |
