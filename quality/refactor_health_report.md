...
| Largest functions by LOC | unknown | 60 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1497 moved `_calculate_dietz_mwr_result(...)` from `61` to `57` lines, and the largest production functions now measure `60` lines |
...
| Collected tests | 2,035 | 3,253 | measured | `python -m pytest --collect-only -q` |
