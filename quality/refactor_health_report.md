...
| Python LOC | 104,454 | 161,007 | measured | `rg --files -g '*.py'` plus PowerShell line count on this branch |
...
| Largest functions by LOC | unknown | 90 | measured | `quality/function_size_inventory.md` via `scripts/python_function_size_inventory.py`; LP-CR-1425 moved `build_runtime_status_response(...)` from `91` lines out of the top-25 table by isolating recovery-drill and runtime-retention status response projection; the largest production functions are now `_build_hierarchy_period_contribution_result(...)` and `_calculate_returns_series(...)` at `90` lines each |
...
| Collected tests | 2,035 | 3,147 | measured | `python -m pytest --collect-only -q` |
