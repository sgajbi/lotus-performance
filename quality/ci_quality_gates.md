...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production functions are now `retrieve_stateful_attribution_source_input(...)` and `aggregate_attribution_results(...)` at `98` lines each after LP-CR-1416 moved `_build_analytics_surfaces(...)` from `113` to `93` lines | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
