...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production functions are now five functions tied at `72` lines after LP-CR-1451 moved `_build_attribution_supportability_reasons(...)` out of the top-25 table | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
