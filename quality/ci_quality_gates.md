...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production functions are now `calculate_contribution(...)` and `LineageMetadataStore._build_inspection_query_statements(...)` at `80` lines after LP-CR-1434 moved `build_runtime_recovery_snapshot(...)` out of the top-25 table by isolating request/filter projection and snapshot-envelope assembly | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
