...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production function is now `_calculate_returns_series(...)` at `90` lines after LP-CR-1426 moved `_build_hierarchy_period_contribution_result(...)` from `90` lines out of the top-25 table | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
