...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production functions are now three functions tied at `70` lines after LP-CR-1459 moved `StatefulInputService._fetch_portfolio_chunk(...)` to `61` lines | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
