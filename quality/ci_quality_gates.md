...
| Python security scanning | Measured in `quality/python_security_inventory.md` through `scripts/python_security_inventory.py` and `bandit`; current scan has zero high and zero medium findings, with five low-severity medium-confidence findings requiring classification before any blocking gate | Keep report-only until low-severity false-positive handling, exception policy, and CI placement are stable. |
