...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production functions are now six functions tied at `74` lines after LP-CR-1442 moved `_build_stateful_calculated_benchmark_input(...)` from `75` to `59` lines and out of the top-25 table by isolating calculated benchmark composition-window loading and parsing | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
