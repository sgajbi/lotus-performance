...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production function now measures `57` lines after LP-CR-1546 split hierarchical contribution result summary, per-position totals, and hierarchy metadata projection behind focused helpers and left `build_hierarchical_contribution_result(...)` outside the top-30 table | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
