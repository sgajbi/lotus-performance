...
| Test taxonomy | Blocking through `make quality-test-taxonomy-gate`, which runs `scripts/python_test_taxonomy_inventory.py --limit 30 --min-api-runtime-tests 651 --min-contract-governance-tests 128 --max-uncategorized-tests 982`; current AST inventory shows 292 modules, 3,432 source test functions, 651 integration/API/runtime test functions, 128 contract/governance test functions, 324 observability/readiness test functions, 1,549 analytics-domain test functions, and 982 uncategorized test functions | Keep the current measured preservation baseline blocking. Classify domain tests through stable taxonomy rules instead of allowing uncategorized growth. |
...
| Intake item | Decision |
| --- | --- |
| Baseline | `quality/test_taxonomy_inventory.md` records 292 test modules, 3,432 source test functions, 651 API/runtime test functions, 128 contract/governance test functions, and 982 uncategorized test functions. Issue #419 added runtime build-identity tests, issue #422 added analytics-domain XIRR boundary tests, and #420 ratcheted the enforced thresholds to the current API/runtime, contract/governance, and uncategorized preservation baseline after focused gate and wiring tests passed. |
