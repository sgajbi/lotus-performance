...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production function is now `resolve_attribution_request(...)` at `94` lines after LP-CR-1419 moved `build_runtime_retention_history_snapshot(...)` from `97` to `78` lines | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
