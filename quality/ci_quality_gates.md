...
| Test taxonomy | Blocking through `make quality-test-taxonomy-gate`, which runs `scripts/python_test_taxonomy_inventory.py --limit 30 --min-api-runtime-tests 607 --min-contract-governance-tests 111 --max-uncategorized-tests 1148`; current AST inventory shows 289 modules, 3,421 source test functions, 647 integration/API/runtime test functions, 126 contract/governance test functions, 322 observability/readiness test functions, 1,547 analytics-domain test functions, and 982 uncategorized test functions | Keep the accepted breadth floor and tightened uncategorized ceiling blocking. Classify domain tests through stable taxonomy rules instead of allowing uncategorized growth. |
...
| Intake item | Decision |
| --- | --- |
| Baseline | `quality/test_taxonomy_inventory.md` records 289 test modules, 3,421 source test functions, 647 API/runtime test functions, 126 contract/governance test functions, and 982 uncategorized test functions. The enforced thresholds remain the established regression floor/ceiling rather than ratcheting automatically with this evidence refresh. |
