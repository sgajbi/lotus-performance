...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production functions are now `_build_workspace_results_by_period(...)` and `_process_pending_jobs(...)` at `70` lines after LP-CR-1460 moved `build_mwr_response(...)` out of the top-25 table | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
