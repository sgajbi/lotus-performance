...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production functions are now `_build_hierarchy_period_contribution_result(...)` and `_calculate_returns_series(...)` at `90` lines each after LP-CR-1425 moved `build_runtime_status_response(...)` from `91` lines out of the top-25 table | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
