...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production function is now `_build_artifacts(...)` at `89` lines after LP-CR-1427 moved `_calculate_returns_series(...)` from `90` to `49` lines and out of the top-25 table | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
