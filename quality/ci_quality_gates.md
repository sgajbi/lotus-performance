...
| Test taxonomy | Blocking through `make quality-test-taxonomy-gate`, which runs `scripts/python_test_taxonomy_inventory.py --limit 30 --min-api-runtime-tests 656 --min-contract-governance-tests 136 --max-uncategorized-tests 825`; current AST inventory shows 324 modules, 3,687 source test functions, 703 integration/API/runtime test functions, 183 contract/governance test functions, 429 observability/readiness test functions, 242 quality/security test functions, 1,674 analytics-domain test functions, and 825 uncategorized test functions | Keep the current measured preservation baseline blocking. Classify domain tests through stable taxonomy rules instead of allowing uncategorized growth. Issue #475 classified the workspace analytics surface and re-banked the ceiling from `969` to the measured `879`; #504 classified the compute executor worker, which sat outside the token list while its own job store and its sibling lineage worker were inside it, and re-banked from `876` to the measured `825`. `runtime` and `operator` remain the two largest unclassified groups. |
...
Current governed inventory: `quality/test_taxonomy_inventory.md` records 324 test modules, 3,687
source test functions, 703 API/runtime test functions, 183 contract/governance test functions, and
825 uncategorized test functions. The full family summary is 324 modules, 3,687 source test
functions, 703 integration/API/runtime test functions, 183 contract/governance test functions, 429
observability/readiness test functions, 242 quality/security test functions, 1,674 analytics-domain
test functions, and 825 uncategorized test functions.

`quality/test_taxonomy_inventory.md` records 324 test modules, 3,687 source test functions, 703 API/runtime test functions, 183 contract/governance test functions, and 825 uncategorized test functions.
