...
| Test taxonomy | Blocking through `make quality-test-taxonomy-gate`, which runs `scripts/python_test_taxonomy_inventory.py --limit 30 --min-api-runtime-tests 655 --min-contract-governance-tests 129 --max-uncategorized-tests 969`; current AST inventory shows 296 modules, 3,440 source test functions, 655 integration/API/runtime test functions, 129 contract/governance test functions, 338 observability/readiness test functions, 142 quality/security test functions, 1,549 analytics-domain test functions, and 969 uncategorized test functions | Keep the current measured preservation baseline blocking. Classify domain tests through stable taxonomy rules instead of allowing uncategorized growth. |
...
| Intake item | Decision |
| --- | --- |
| Baseline | `quality/test_taxonomy_inventory.md` records 296 test modules, 3,440 source test functions, 655 API/runtime test functions, 129 contract/governance test functions, and 969 uncategorized test functions. Issue #419 added runtime build-identity tests, issue #422 added analytics-domain XIRR boundary tests, #420 ratcheted the enforced thresholds to the current API/runtime, contract/governance, and uncategorized preservation baseline, #423 classified config/resilience tests so the uncategorized ceiling could tighten without losing coverage, #442 classified async polling cadence tests as API/runtime evidence, and #454 classified core analytics footer parity as API/runtime plus contract/governance evidence. |
