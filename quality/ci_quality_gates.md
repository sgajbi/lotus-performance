...
| Pull Request Merge Gate | Pull requests targeting `main` and manual dispatch | workflow lint, static quality gates, contract/security gates, domain-product validation, deterministic API evaluation, test taxonomy breadth, compatibility `Lint Typecheck Security` aggregate, lineage-volume restart recovery, migration smoke, unit, integration, and e2e tests, combined coverage floor at 99 percent, Docker build, container SBOM and vulnerability-report artifact publication |
| Main Releasability Gate | Pushes to `main` and manual dispatch | workflow lint, static quality gates, contract/security gates, domain-product validation, deterministic API evaluation, test taxonomy breadth, lineage-volume restart recovery, migration smoke, unit, integration, and e2e tests, combined coverage floor at 99 percent, coverage artifact publication, Docker build, container SBOM/vulnerability artifact publication, SBOM provenance attestation |
...
the split static-quality, contract-security, and lineage-volume-recovery jobs. It exists to satisfy
the current GitHub required-check contract while preserving the faster parallel lane structure.
...
| Test taxonomy | Blocking through `make quality-test-taxonomy-gate`, which runs `scripts/python_test_taxonomy_inventory.py --limit 30 --min-api-runtime-tests 656 --min-contract-governance-tests 136 --max-uncategorized-tests 797`; current AST inventory shows 327 modules, 3,775 source test functions, 754 integration/API/runtime test functions, 188 contract/governance test functions, 431 observability/readiness test functions, 264 quality/security test functions, 1,723 analytics-domain test functions, and 797 uncategorized test functions | Keep the current measured preservation baseline blocking. Classify domain tests through stable taxonomy rules instead of allowing uncategorized growth. Issue #475 classified the workspace analytics surface and re-banked the ceiling from `969` to the measured `879`; #504 classified the compute executor worker, which sat outside the token list while its own job store and its sibling lineage worker were inside it, and this Cycle 5 slice classifies async-result and execution-polling access as API/runtime plus applied-currency evidence as analytics while ratcheting the ceiling to the measured tree. `runtime` and `operator` remain the two largest unclassified groups. |
| PostgreSQL concurrency contracts | Blocking inside `PR Merge Gate / Tests (integration)` through `make postgres-concurrency-contracts-gate`, which runs `scripts/postgres_concurrency_contracts_gate.py` and decides from JUnit XML counts rather than pytest's printed summary: it refuses a skip, a nonzero pytest exit, an empty collection, and any failure or error, and neutralizes any inherited pytest selection (`-o addopts=` plus dropping `PYTEST_ADDOPTS`) because a deselected contract is absent from the report rather than counted as skipped. Local command: **`make postgres-concurrency-contracts-local`**, which provisions the `performance-lineage-db` compose service first — the gate needs a live PostgreSQL and the local target builds the DSN from `PA_LINEAGE_DB_PORT` so a custom compose port still reaches the database, and an explicit DSN still wins. Override with `LOTUS_POSTGRES_PLAN_DATABASE_URL` to point at another database | Keep blocking. The proofs `pytest.skip` when no database answers, so a skip and a pass are the same colour to a lane — the gate exists to tell them apart. Remediation for a refusal: start the database (`make postgres-concurrency-contracts-local`) rather than relaxing the gate; a skipped advisory-lock proof is the defect #489 was opened about. |
| License compliance | Blocking through `make license-compliance-gate`, which validates `contracts/license-compliance-policy.v1.json` against `quality/license_compliance_inventory.md`; current inventory covers 47 runtime, development and image packages, with 45 allowed packages, 2 review-required packages covered by active exceptions, 0 blocked packages, and 0 missing-exception findings. The image source is `requirements-image.txt`, a single governed manifest the Dockerfile and the development environment both install from, so a package the image ships cannot be absent from the inventory | Keep blocking. Regenerate with `python scripts/license_compliance_inventory.py --write` after dependency changes, and keep exceptions owner-bound and time-bound before release. |
...
| Lineage volume restart recovery | Blocking in PR and main lanes through `make lineage-volume-recovery-smoke`; the PR job feeds the required `Lint Typecheck Security` aggregate | Keep blocking. The proof must seed root-owned evidence, repair to UID/GID `10001`, validate non-root health and write access, retain evidence across restart, and clean only its generated Compose project. |
| Container supply-chain evidence | Blocking gate plus artifact production in PR and main lanes through `make container-supply-chain-evidence`; builds the non-root production runtime image with non-secret Git/build metadata, writes a CycloneDX SBOM and high/critical Trivy vulnerability JSON under ignored `output/container-security/`, exposes the matching runtime identity shape at `/version`, and Main Releasability attests SBOM provenance with `actions/attest-build-provenance@v3` | Keep artifact publication mandatory. `make container-vulnerability-gate` is promoted and blocking in both lanes; accepted high/critical exceptions are owner-bound, time-bound, package-identity-bound and validated against the same scan by `scripts/container_acceptance_gate.py`, which `make container-vulnerability-gate` runs. |
| Performance characterization evidence | Evidence-producing workflow in `.github/workflows/performance-characterization.yml`; pull requests to `main`, pushes to `main`, weekly schedule, and manual dispatch run `make performance-characterization` with a live PostgreSQL service, rerun the PostgreSQL plan/concurrency subset with `--require-non-skipped`, and upload logs, JUnit XML, and summary JSON from `output/performance-characterization/` | Keep this as a report/evidence lane unless the benchmark suite proves stable enough to become a required check. Do not soft-fail benchmark budget regressions or all-skipped PostgreSQL contracts; workflow artifacts are current characterization evidence, not a replacement for the faster unit/integration/e2e merge gates. |
...
| Observability and operational contracts | Blocking through `make quality-observability-readiness-gate`, which runs `scripts/python_observability_readiness_inventory.py --limit 30 --max-missing 0`; current report shows 28/28 expected implementation markers, 0 missing markers, 481 family-mapped readiness test functions, 14 deployable alert rules, 12 dashboard panels, and 0 monitoring artifact violations | Keep the zero-missing marker and zero monitoring-artifact-violation gate blocking in feature, PR, and main static quality lanes. The gate also checks alert-backed product supportability dashboard coverage so an expected product alert cannot pass with only a generic panel-count baseline. Broader maturity scoring and overlap-aware test counting remain report-only planning evidence. |
...
This document deliberately mixes regenerated figures with hand-written prose. The table row above
is refreshed from `scripts/python_test_taxonomy_inventory.py`; the paragraph below is written by
hand. A refresh that replaces exact phrases updates one and not the other, especially where a total
wraps across a line break, which is how a superseded count survived once already. Both are now
checked by `test_current_inventory_prose_carries_no_stale_totals`, which normalises wrapping and
...
Current governed inventory: `quality/test_taxonomy_inventory.md` records 327 test modules, 3,775
source test functions, 754 API/runtime test functions, 188 contract/governance test functions, and
797 uncategorized test functions. The full family summary is 327 modules, 3,775 source test
functions, 754 integration/API/runtime test functions, 188 contract/governance test functions, 431
observability/readiness test functions, 264 quality/security test functions, 1,723 analytics-domain
test functions, and 797 uncategorized test functions.

`quality/test_taxonomy_inventory.md` records 327 test modules, 3,775 source test functions, 754 API/runtime test functions, 188 contract/governance test functions, and 797 uncategorized test functions.
...
| Exception policy | `make container-vulnerability-gate` is blocking with no `continue-on-error`. Exceptions are narrow, owner-bound and time-bound, recorded in `quality/container_vulnerability_acceptances.v1.json` with package identity, affected version, severity and remediation path, and validated against the same scan by `scripts/container_acceptance_gate.py`, which `make container-vulnerability-gate` runs, which refuses anything fixable, drifted, or expired. |
...
## LP-CR-1681 Performance Characterization Evidence Intake

| Intake item | Decision |
| --- | --- |
| Baseline | GitHub issue #426 identified that `make performance-characterization` and the live PostgreSQL plan/concurrency contracts existed locally but were not executed by any GitHub Actions lane, leaving benchmark evidence invisible to reviewers. |
| Failure mode addressed | Agents could update performance-sensitive code, docs, or SLO language while relying on local-only benchmark claims and silently skipped PostgreSQL checks. |
| Determinism | `scripts/run_performance_characterization.py` delegates to the existing benchmark tests, writes JUnit/log/summary artifacts under ignored `output/performance-characterization/`, waits for PostgreSQL readiness in PostgreSQL mode, and fails closed when the selected PostgreSQL suite only skips. |
| Lane placement | Dedicated Performance Characterization Evidence workflow on pull requests to `main`, pushes to `main`, weekly schedule, and manual dispatch. The lane provisions a real PostgreSQL service and uploads artifacts; it is not configured as a required merge check yet. |
| Exception policy | Do not use `continue-on-error`. PostgreSQL characterization must either run against the live service or fail with artifact evidence; all-skipped PostgreSQL output is not accepted as proof. |
| Focused tests | `tests/unit/scripts/test_ci_quality_gate_wiring.py` proves Make target wiring, live PostgreSQL service configuration, workflow target calls, artifact upload, and no soft-fail posture. |
| Scorecard and ledger truth | `docs/technical/performance_characterization.md`, this gate map, `quality/quality_scorecard.md`, repo-local wiki source, repository context, the issue matrix, and the codebase review ledger record the lane. |

## LP-CR-1540 Gate Promotion Intake

| Intake item | Decision |
| --- | --- |
| Baseline | `quality/test_taxonomy_inventory.md` records 305 test modules, 3,526 source test functions, 690 API/runtime test functions, 149 contract/governance test functions, and 954 uncategorized test functions. The same inventory records 165 quality/security test functions. Issue #419 added runtime build-identity tests, issue #422 added analytics-domain XIRR boundary tests, #420 ratcheted the enforced thresholds to the current API/runtime, contract/governance, and uncategorized preservation baseline, #423 classified config/resilience tests so the uncategorized ceiling could tighten without losing coverage, #442 classified async polling cadence tests as API/runtime evidence, #454 classified core analytics footer parity as API/runtime plus contract/governance evidence, #453 classified strict fail-fast parity across completed core analytics endpoints, #417 classified runtime/static trust-telemetry evidence posture, #424 classified executable durable schema apply/verify coverage without growing uncategorized tests, #425 classified returns-series strict-intersection fill semantics coverage without growing uncategorized tests, #452 classified RFC-020 support-matrix docs governance without growing uncategorized tests, #451 classified RFC-021 gross/net support-baseline docs governance without growing uncategorized tests, #449 classified governed operator calculation-id prefix lookup coverage without growing uncategorized tests, #448 classified lineage-worker lifecycle tests as observability/readiness evidence while preserving the `969` uncategorized blocking ceiling, #447 classified stateful portfolio source port contract tests as analytics-domain evidence without growing uncategorized tests, #446 classified lineage artifact classification route tests as API/runtime evidence while staying below the uncategorized ceiling, #444 added runtime-retention restart-safety and failed-replay tests while staying below the uncategorized ceiling, #443 added async SLO/capacity contract docs coverage, #441 added upstream dependency inventory contract coverage while ratcheting the contract/governance floor to the measured count, #440 added license compliance gate tests while raising quality/security coverage, #439 added restore-validation drill coverage while staying below the uncategorized ceiling, #438 added durable database engine policy coverage while keeping the uncategorized backlog flat, #436 added runtime-retention legal-hold coverage while preserving the uncategorized ceiling, #435 added governed MARKET calendar coverage while preserving the uncategorized ceiling, #434 added retrieval metadata anti-corruption coverage while staying below the uncategorized ceiling, #433 classified stateful execution policy and submission fencing tests as API/runtime evidence while reducing uncategorized tests, #432 added container runtime contract tests without growing uncategorized tests, #431 added calculation-engine-version policy/static-gate coverage plus public-doc guards while keeping API/runtime and uncategorized counts stable, #430 added ordered-array hash-contract API/runtime and public-doc governance coverage while staying below the uncategorized ceiling, #429 added operator-navigation API path coverage for workspace-summary runtime result links while keeping uncategorized tests flat, #428 added runtime settings-validation and production threshold fail-closed coverage without growing uncategorized tests, #426 added performance-characterization CI, durable PostgreSQL option, and lineage-inspection hot-path coverage while staying below the uncategorized ceiling, #427 added alert-dashboard coverage regression proof while preserving the uncategorized ceiling, and the PR review fixes added legacy-lineage and non-finite retrieval metadata regression proof while preserving the ceiling. |
