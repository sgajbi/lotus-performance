...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production functions are now two functions tied at `72` lines after LP-CR-1455 moved the contribution period result builders down to `67` and `64` lines | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
