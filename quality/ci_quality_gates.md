...
| Duplicate code hotspots | Blocking through `make quality-duplicate-code-gate`; current report shows 12 duplicate hotspot groups at `--min-lines 12` with `--max-groups 12` | Keep blocking against the current measured baseline while refactor slices reduce duplicate hotspots; future increases require a documented reason and a better reusable abstraction decision. |
