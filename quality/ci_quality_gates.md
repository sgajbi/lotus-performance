...
| Test taxonomy | Blocking through `make quality-test-taxonomy-gate`, which runs `scripts/python_test_taxonomy_inventory.py --limit 30 --min-api-runtime-tests 607 --min-contract-governance-tests 111 --max-uncategorized-tests 1294`; current AST inventory shows 281 modules, 3,223 source test functions, 608 integration/API/runtime test functions, 111 contract/governance test functions, 250 observability/readiness test functions, 1,126 analytics-domain test functions, and 1,239 uncategorized test functions | Keep the accepted breadth floor and uncategorized ceiling blocking. Reduce uncategorized tests through normal refactor slices before tightening the ceiling. |
