...
| Test taxonomy | Blocking through `make quality-test-taxonomy-gate`, which runs `scripts/python_test_taxonomy_inventory.py --limit 30 --min-api-runtime-tests 607 --min-contract-governance-tests 111 --max-uncategorized-tests 1294`; current AST inventory shows 280 modules, 3,209 source test functions, 608 integration/API/runtime test functions, 111 contract/governance test functions, 189 observability/readiness test functions, and 1,294 uncategorized test functions | Keep the accepted breadth floor blocking. Reduce uncategorized tests through normal refactor slices before tightening the ceiling. |
...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production functions now measure `57` lines after LP-CR-1544 split stateful contribution normalized input assembly into portfolio and position projection helpers and left `build_stateful_contribution_input(...)` outside the top-30 table | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
