...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production function is now `build_runtime_status_response(...)` at `91` lines after LP-CR-1424 moved `_build_flat_period_contribution_result(...)` from `92` lines out of the top-25 table | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
