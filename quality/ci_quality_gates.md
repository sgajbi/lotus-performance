...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production function remains `68` lines after LP-CR-1466 moved `StatefulInputService._fetch_position_chunk(...)` from `68` to `65` lines | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
