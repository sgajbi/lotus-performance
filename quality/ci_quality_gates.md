...
| Test taxonomy | Blocking through `make quality-test-taxonomy-gate`, which runs `scripts/python_test_taxonomy_inventory.py --limit 30 --min-api-runtime-tests 607 --min-contract-governance-tests 111 --max-uncategorized-tests 1148`; current AST inventory shows 281 modules, 3,224 source test functions, 608 integration/API/runtime test functions, 111 contract/governance test functions, 250 observability/readiness test functions, 1,264 analytics-domain test functions, and 1,148 uncategorized test functions | Keep the accepted breadth floor and tightened uncategorized ceiling blocking. Classify domain tests through stable taxonomy rules instead of allowing uncategorized growth. |
...
| Function-size hotspots | Measured in `quality/function_size_inventory.md` through a repo-native standard-library scanner; largest production functions now measure `56` lines after LP-CR-1549 split compute-job inspection statement routing behind reclaimable and standard-status helpers and left `ComputeJobStore._build_inspection_statements(...)` outside the top-30 table | Use as refactor-planning evidence; do not block CI until stable thresholds and exclusions are agreed. |
