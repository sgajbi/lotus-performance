...
```powershell
python scripts/python_function_size_inventory.py --limit 45
```
...
| ---: | --- | --- | ---: |
| 1 | `StatefulInputService._fetch_position_chunk` | `app/services/stateful_input_service.py:1159` | 81 |
| 2 | `StatefulInputService._fetch_portfolio_chunk` | `app/services/stateful_input_service.py:1009` | 78 |
| 3 | `_handle_compute_job_failure` | `app/workers/compute_executor_worker.py:576` | 66 |
| 4 | `_source_mwr_cash_flow_component` | `app/services/stateful_mwr_input_service.py:353` | 63 |
| 5 | `calculate_mwr_response` | `app/services/mwr_calculation_service.py:269` | 62 |
| 6 | `resolve_attribution_request` | `app/services/attribution_mode_service.py:37` | 57 |
| 7 | `calculate_workspace_summary_async` | `app/services/workspace_summary_service.py:144` | 57 |
| 8 | `resolve_benchmark_request` | `app/services/benchmark_mode_service.py:87` | 56 |
| 9 | `LineageMetadataStore._ensure_payload_lease_columns` | `app/services/lineage_metadata_store.py:1127` | 56 |
| 10 | `calculate_money_weighted_return` | `engine/mwr.py:734` | 56 |
| 11 | `_build_hierarchy_period_contribution_result` | `app/services/contribution_service.py:480` | 55 |
| 12 | `build_stateful_mwr_input_for_window` | `app/services/stateful_mwr_input_service.py:136` | 55 |
| 13 | `_build_methodology_shadow_samples` | `engine/compute.py:319` | 55 |
| 14 | `build_enterprise_audit_middleware` | `app/enterprise_audit_middleware.py:69` | 54 |
| 15 | `_run_subject_assessments` | `app/services/inspection/twr_inspection_service.py:207` | 54 |
| 16 | `_build_workflow_capabilities` | `app/services/integration_capabilities_service.py:222` | 54 |
| 17 | `run_runtime_retention_cleanup` | `app/services/runtime_retention_run_service.py:150` | 54 |
| 18 | `lineage_queue_degradation_details` | `app/services/runtime_status_degradation.py:75` | 54 |
| 19 | `run_calculations` | `engine/compute.py:26` | 54 |
| 20 | `_xirr` | `engine/mwr.py:260` | 54 |
| 21 | `calculate_attribution` | `app/services/attribution_service.py:368` | 53 |
| 22 | `_build_contribution_response` | `app/services/contribution_service.py:713` | 53 |
| 23 | `_build_contribution_response_evidence` | `app/services/contribution_service.py:768` | 53 |
| 24 | `_build_flat_period_contribution_result` | `app/services/contribution_service.py:425` | 53 |
| 25 | `analyze_portfolio_position_reconciliation` | `app/services/inspection/reconciliation.py:235` | 53 |
| 26 | `_build_explicit_amount_contract_findings` | `app/services/inspection/source_economics_findings.py:380` | 53 |
| 27 | `compute_queue_degradation_breach_metric` | `app/services/queue_metric_builders.py:185` | 53 |
| 28 | `_build_returns_series_diagnostics` | `app/services/returns_series_service.py:1275` | 53 |
| 29 | `finalize_twr_resolved_execution_identity` | `app/services/twr_calculation_service.py:148` | 53 |
...
| 32 | `_build_contribution_smoothing_evidence` | `app/services/contribution_smoothing.py:119` | 52 |
| 33 | `_build_workflow_pack_request` | `app/services/inspection/support_brief_workflow_pack.py:88` | 52 |
| 34 | `_build_analytics_surfaces` | `app/services/integration_capabilities_service.py:448` | 52 |
| 35 | `_build_mwr_response_payload` | `app/services/mwr_calculation_service.py:143` | 52 |
| 36 | `_calculate_promoted_stateful_returns_series` | `app/services/returns_series_calculation_workflow_service.py:170` | 52 |
| 37 | `_build_resolved_stateful_returns_series_request` | `app/services/returns_series_service.py:925` | 52 |
| 38 | `_resolve_promoted_stateful_contribution_response` | `app/services/contribution_calculation_workflow_service.py:137` | 51 |
| 39 | `_build_hierarchy_contribution_position_assembly` | `app/services/contribution_service.py:372` | 51 |
| 40 | `_calculate_returns_series` | `app/services/returns_series_service.py:1633` | 51 |
| 41 | `_available_runtime_work_item_snapshot` | `app/services/runtime_work_item_service.py:110` | 51 |
| 42 | `StatefulInputService._fetch_performance_component_economics_chunk` | `app/services/stateful_input_service.py:1334` | 51 |
| 43 | `_build_twr_period_result` | `app/services/twr_service.py:894` | 51 |
| 44 | `_build_stateful_workspace_benchmark_input_async` | `app/services/workspace_summary_service.py:465` | 51 |
| 45 | `_build_currency_attribution_projection` | `engine/attribution.py:781` | 51 |

## Interpretation

This is not a cyclomatic-complexity score. It is a deterministic hotspot inventory for refactor
planning. The largest current functions are concentrated in stateful source retrieval, compute
worker failure handling, stateful MWR source projection, MWR response assembly, and mode-resolution
helpers. LP-CR-1638 isolated shared pagination-failure snapshot recording from portfolio and
position chunk traversal, moving `StatefulInputService._fetch_position_chunk(...)` from `85` to
`81` lines and `_fetch_portfolio_chunk(...)` from `82` to `78` lines while preserving retrieval
behavior and snapshot evidence.
TWR workflow assembly has dropped out of the top-15 table after resolved identity finalization was isolated.
...
isolated.
LP-CR-1548 isolated Dietz MWR result assembly into
`_zero_denominator_dietz_mwr_result(...)` and `_calculated_dietz_mwr_result(...)`, leaving
`_calculate_dietz_mwr_result(...)` focused on Dietz return-component calculation,
zero-denominator routing, fallback metadata selection, and result-builder dispatch. The former
top-ranked `_calculate_dietz_mwr_result(...)` dropped out of the top-30 table; the largest
production functions now start at `56` lines. This is an internal design-modularity and MWR-engine
readability slice only; it adds no runtime microservice or worker boundary.
LP-CR-1549 isolated compute-job inspection statement routing into reclaimable and standard-status
statement assembly helpers. The former top-ranked `ComputeJobStore._build_inspection_statements(...)`
dropped out of the top-30 table; the largest production functions still start at `56` lines, now led
by `run_twr_inspection(...)`. This is an internal persistence-boundary and operator-inspection
readability slice only; it preserves query semantics and adds no runtime microservice or worker
boundary.
LP-CR-1552 introduced `_StatefulContributionSourceRequest` so portfolio input, Core position
timeseries, and optional `PerformanceComponentEconomics:v1` retrieval share one typed private source
request instead of repeating the same portfolio, window, currency, consumer, dimension, cash-flow,
and filter arguments across helpers. `retrieve_stateful_contribution_source_input(...)` dropped out
of the top-35 table; the largest production functions still start at `56` lines. This is an internal
source-boundary design-modularity slice only; it preserves public contribution API behavior and adds
no runtime microservice or worker boundary.
LP-CR-1553 introduced `_RuntimeRetentionHistoryQuery` so runtime-retention history filtering,
pagination, and applied-filter projection move through one private query carrier instead of repeated
argument plumbing across manifest-unavailable and manifest-available snapshot paths.
`build_runtime_retention_history_snapshot(...)` dropped out of the top-35 table; the largest
...
LP-CR-1555 introduced `_WorkspacePerformanceBreakdownWindow`,
...
has a named window boundary for bucket slicing, cumulative daily-result selection, economics
projection, and annualized return projection. `_build_workspace_performance_breakdowns(...)`
dropped out of the top-40 table; the largest production functions still start at `56` lines. This
is an internal workspace-summary design-modularity slice only; it preserves workspace summary API,
benchmark, active-return, MWR, and OpenAPI behavior and adds no runtime microservice or worker
boundary.
LP-CR-1556 introduced `_WorkspaceSummaryProjection` and
`_build_workspace_summary_projection(...)` so workspace summary response assembly delegates
valuation-date normalization, net/gross daily-result normalization, benchmark daily-frame
projection, requested-frequency projection, and period-result construction to a named private
projection boundary. `_build_workspace_summary_response(...)` dropped out of the top-35 table;
the largest production functions still start at `56` lines. This is an internal workspace-summary
design-modularity slice only; it preserves workspace summary API, benchmark, active-return, MWR,
diagnostics, audit, OpenAPI, error-model, observability, and runtime behavior and adds no runtime
microservice or worker boundary.
LP-CR-1557 isolated XIRR attempt result projection into `_xirr_attempt_convergence(...)`,
`_successful_xirr_mwr_attempt(...)`, `_not_applicable_xirr_mwr_attempt(...)`, and
`_fallback_xirr_mwr_attempt(...)`, leaving `_calculate_xirr_mwr_attempt(...)` focused on solver
execution and outcome dispatch. `_calculate_xirr_mwr_attempt(...)` dropped out of the top-45 table;
the largest production functions now start at `55` lines. This is an internal MWR-engine
design-modularity slice only; it preserves XIRR solver inputs, success projection,
no-economic-content `NOT_APPLICABLE` behavior, Modified Dietz fallback signaling, public MWR API,
OpenAPI, error-model, observability, and runtime behavior and adds no runtime microservice or worker
boundary.
LP-CR-1558 isolated durable queue metric source loading into core queue, recovery-drill, and
runtime-retention source-loader boundaries. `_load_durable_queue_metric_sources(...)` dropped out of
the top-45 table; the largest production functions still start at `55` lines. This is an internal
observability collector design-modularity slice only; it preserves Prometheus metric names, labels,
availability semantics, runtime-retention preview degradation behavior, API/OpenAPI truth, and
runtime topology and adds no runtime microservice or worker boundary.
LP-CR-1559 isolated returns-series execution frame normalization into
`_normalize_returns_series_execution_frames(...)` and stage detail projection into
`_returns_series_execution_stage_details(...)`, leaving `_build_returns_series_execution_result(...)`
focused on point projection, identity finalization, diagnostics, and response assembly.
`_build_returns_series_execution_result(...)` dropped out of the top-45 table; the largest production
functions still start at `55` lines. This is an internal returns-series design-modularity slice only;
it preserves strict intersection, fill policy, benchmark context, provenance, diagnostics, API,
OpenAPI, error-model, observability, and runtime behavior and adds no runtime microservice or worker
boundary.
LP-CR-1560 isolated runtime recovery compute/lineage queue-result loading into
`_runtime_recovery_queue_results(...)` and `_RuntimeRecoveryQueueResults`, leaving
`build_runtime_recovery_snapshot(...)` focused on generated-at selection, durable metadata
readiness, request projection, and snapshot envelope assembly. `build_runtime_recovery_snapshot(...)`
dropped out of the top-45 table; the largest production functions still start at `55` lines. This
is an internal runtime-supportability design-modularity slice only; it preserves runtime recoveries
API behavior, queue filters, cursor semantics, durable metadata outage handling, partial queue
degradation, OpenAPI/error-model truth, and runtime topology, and adds no runtime microservice or
worker boundary.
LP-CR-1561 introduced `_StatefulAttributionSourceRetrievalRequest` and
`_stateful_attribution_source_input_from_bundle(...)` so stateful attribution source retrieval has a
single private request carrier and a named public DTO projection boundary. The former top-ranked
`_retrieve_stateful_attribution_sources(...)` dropped out of the top-45 table; the largest
...
typed source request, named chunk-fetch boundary, named response projection, and test-backed
snapshot payload builder. The former top-ranked
`StatefulInputService.get_benchmark_market_series(...)` dropped out of the top-45 table; the
largest production functions still start at `55` lines. This is an internal stateful benchmark
source-boundary slice only; it preserves lotus-core source authority, chunking, default
`series_fields`, snapshot identity, first-failure behavior, API/OpenAPI, error-model, and runtime
behavior and adds no runtime microservice or worker boundary.
...
`build_hierarchical_contribution_result(...)` focused on empty-result routing, Carino residual
allocation, hierarchy response-level construction, and final response assembly. The former
top-ranked `build_hierarchical_contribution_result(...)` dropped out of the top-30 table; the
largest production function is now `_calculate_dietz_mwr_result(...)` at `57` lines. This is an
internal design-modularity and contribution-engine readability slice only; it adds no runtime
microservice or worker boundary.
...
position metadata, currency-mode validation, source-economics metadata, and normalized contribution
request shape. `build_stateful_contribution_input(...)` dropped out of the top-30 table; the
largest production functions are now three functions tied at `57` lines, led by
`StatefulInputService._fetch_position_chunk(...)`. This is an internal design-modularity and
contribution source-boundary slice only; it adds no runtime microservice or worker boundary.
LP-CR-1543 isolated runtime-retention status snapshot routing into
`_runtime_retention_status_from_unavailable_history(...)` and
`_runtime_retention_status_from_latest_history(...)`, preserving missing-history semantics,
unavailable-history reasons, preview fields, active governed-action context, latest retained
cleanup fields, degradation reasons/details, and runtime-retention status response shape.
`runtime_retention_status_from_snapshot(...)` dropped out of the top-30 table; the largest
production functions are now four functions tied at `57` lines, led by
`build_stateful_contribution_input(...)`. This is an internal design-modularity and operational
readiness slice only; it adds no runtime microservice or worker boundary.
LP-CR-1542 isolated period-level TWR calculation-consistency orchestration into
`_check_period_relative_consistency(...)` and
`_check_period_linked_block_consistency(...)`, preserving relative-performance pairing/order,
linked-block counters, daily calculation evidence counters, finding codes, and evidence payload
shape. `_check_period_calculation_consistency(...)` dropped out of the top-30 table, and the
largest production functions are now five functions tied at `57` lines, led by
`runtime_retention_status_from_snapshot(...)`. This is a design-modularity and inspection-domain
supportability slice only; it adds no runtime service boundary.
...
`build_performance_diagnostics(...)` dropped out of the top-30 table, and the largest production
functions are now six functions tied at `57` lines, led by
`_check_period_calculation_consistency(...)`. Behavior is unchanged: TWR and workspace summary
responses still preserve engine diagnostics, policy counts, outlier samples, methodology-shadow
samples, reset events, OpenAPI schemas, and existing effective-period fail-fast behavior. This is a
design-modularity and domain-ownership slice only; it adds no runtime service boundary.
LP-CR-1535 isolated execution polling retrieval and durable-record response projection into
`app/services/execution_polling_service.py`, leaving `app/models/execution_polling.py` as
schema-only API contract models and reducing the router to endpoint lookup plus legacy 404 error
mapping. `build_execution_response(...)` dropped out of the top-30 table, and the largest
production functions are now seven functions tied at `57` lines, led by the former
`build_performance_diagnostics(...)` hotspot. Behavior is unchanged: execution polling still preserves
execution identity, stages, upstream snapshots, compute-job metadata, async-result metadata,
OpenAPI response schemas, and the typed legacy `ErrorDetailResponse` 404 contract. This is a
design-modularity and API/error-contract ownership slice only; it adds no runtime service boundary.
...
function is now `build_recovery_drill_history_snapshot(...)` at `58` lines. Behavior is unchanged:
durable queue metrics still preserve recovery-drill and runtime-retention policy thresholds,
governed action lease metrics, latest-history age metrics, degradation-breach reason labels, and
available-snapshot gating. This is a design-modularity slice only; it adds no runtime service
boundary.
LP-CR-1532 isolated contribution portfolio-engine diagnostic state construction and public
diagnostics envelope projection into focused helpers. `_build_portfolio_engine_diagnostics(...)`
dropped out of the top-30 table, and the largest production functions are now two functions tied at
`58` lines. Behavior is unchanged: contribution diagnostics still preserve private-banking
reset/NIP characterization, candidate canonical reset counts, reset-delta evidence, reset-relative
day counts, effective-period identity, and the existing public `Diagnostics` envelope shape. This
is a design-modularity slice only; it adds no runtime service boundary.
LP-CR-1531 isolated TWR inspection position-reconciliation evidence projection into
`app/services/inspection/reconciliation_result.py`. `_build_position_reconciliation_result(...)`
dropped out of the top-30 table, `app/services/inspection/reconciliation.py` maintainability
improved, and the largest production functions are now three functions tied at `58` lines. This is
a design-modularity slice only; it adds no runtime service boundary.
LP-CR-1519 isolated performance component-economics aggregate supportability policy from payload
assembly into focused state/reason and missing-family helpers. Function-size posture is unchanged;
the current largest production function is `retrieve_stateful_contribution_source_input(...)` at
`63` lines, while `_build_performance_component_economics_payload(...)` no longer appears in the
top-25 complexity inventory.
LP-CR-1520 isolated stateful contribution source-economics reason-code projection into focused
...
LP-CR-1523 isolated recovery-drill history query projection into a FastAPI dependency module,
mirroring the existing runtime-retention history query boundary. `get_recovery_drill_history(...)`
...
LP-CR-1525 isolated compute queue stats aggregate SQL column construction into named helpers.
`ComputeJobStore._build_queue_stats_statement(...)` dropped out of the top-25 table, and the
largest production functions are now five functions tied at `59` lines. Behavior is unchanged:
runtime status and operator metrics still use one aggregate query and preserve pending, leased,
running, failed, complete, retry backlog, lease-expired, reclaimable, terminal failure, and oldest
age fields.
LP-CR-1526 isolated compute recovery query filter normalization and page construction into focused
helpers shared by the item and count statements. `ComputeJobStore.list_recent_recoveries(...)`
dropped out of the top-25 table, and the largest production functions are now four functions tied
at `59` lines. Behavior is unchanged: runtime/operator recovery inspection still preserves
analytics-type, calculation-handle substring, UTC recovered-at window, seek-cursor, total-count,
next-offset, and cursor pagination semantics.
LP-CR-1527 isolated calculated benchmark source loading and source-detail projection into
`_load_calculated_benchmark_sources(...)` and `_calculated_benchmark_source_details(...)`.
`_build_stateful_calculated_benchmark_input(...)` dropped out of the top-25 table, and the
largest production functions are now three functions tied at `59` lines. Behavior is unchanged:
stateful calculated benchmark input still preserves deterministic component retrieval, composition
segments, price-series metadata, FX metadata, normalized component observations, benchmark
currency, and empty vendor-return points for calculated mode.
LP-CR-1528 isolated completed TWR supportability/benchmark projection, execution-stage lifecycle,
benchmark return-source normalization, and period-result projection into focused helpers.
`_assemble_completed_twr_response(...)` and `calculate_twr_response(...)` dropped out of the
top-30 table, and the largest production function is now
`build_attribution_supportability_evidence(...)` at `59` lines. Behavior is unchanged: completed
TWR responses still preserve benchmark context, calculation supportability, bounded metric labels,
lineage completion, period results, and execution-stage failure recording.
LP-CR-1529 isolated attribution supportability evidence into an explicit projection boundary.
`build_attribution_supportability_evidence(...)` dropped out of the top-30 table, and the largest
production functions are now five functions tied at `58` lines. Behavior is unchanged:
attribution supportability still preserves private-banking benchmark coverage gaps, unclassified
segment evidence, Karnosky-Singer currency-attribution status, linking status, residual
materiality, reason ordering, and lineage flags.
LP-CR-1530 isolated runtime-retention cleanup evidence projection into a focused helper.
`execute_runtime_retention_cleanup(...)` dropped out of the top-30 table, and the largest
production functions are now four functions tied at `58` lines. Behavior is unchanged:
operator-triggered runtime-retention cleanup still preserves identity normalization, dry-run/apply
status semantics, prunable object counts, evidence file naming, and retained history persistence.
...
LP-CR-1491 isolated stateful MWR market-value evidence projection into
`_stateful_mwr_market_value_evidence(...)`. `build_stateful_mwr_input_for_window(...)` dropped out
of the top-25 table, duplicate hotspots remain `0`, and the largest production functions now
measure `61` lines.
LP-CR-1492 isolated benchmark market-series retrieval and component-series response mapping into
`_retrieve_benchmark_component_series(...)`. `build_benchmark_exposure_context(...)` dropped out of
the top-25 table, duplicate hotspots remain `0`, and the largest production functions continue to
measure `61` lines.
LP-CR-1493 isolated raw TWR inspection response model construction into
`_build_twr_inspection_response_model(...)`. `_build_twr_inspection_response(...)` dropped out of
the top-25 table, duplicate hotspots remain `0`, and the largest production functions continue to
measure `61` lines.
LP-CR-1494 isolated TWR inspection assessment-output aggregation into
`_SubjectAssessmentAggregation.merge(...)`. `_run_subject_assessments(...)` dropped out of the
top-25 table, duplicate hotspots remain `0`, and the largest production functions continue to
measure `61` lines.
LP-CR-1495 isolated lineage manifest writing and completion recording into focused
`LineageService` helpers. `LineageService.materialize_payload(...)` dropped out of the top-25
table, duplicate hotspots remain `0`, and the largest production functions continue to measure
`61` lines.
LP-CR-1496 isolated stateful portfolio chunk response projection into
`StatefulInputService._build_portfolio_chunk_payload(...)`. `_fetch_portfolio_chunk(...)` dropped
out of the top-25 table, duplicate hotspots remain `0`, and the largest production functions
continue to measure `61` lines.
...
lines.
LP-CR-1499 isolated stateful portfolio source-contract projection into
`_stateful_portfolio_input_from_payload(...)`. `retrieve_stateful_portfolio_input(...)` dropped out
of the top-25 table, duplicate hotspots remain `0`, and the largest production functions continue
to measure `60` lines.
LP-CR-1500 isolated lineage recovery time-filter normalization, row-to-event projection, and page
cursor assembly into focused helpers. `LineageMetadataStore.list_recent_recoveries(...)` dropped
out of the top-25 table, duplicate hotspots remain `0`, and the largest production functions
continue to measure `60` lines.
LP-CR-1501 isolated TWR request mode dispatch, stateless projection, retrieval-stage execution,
and normalization-stage execution into focused helpers. `resolve_twr_request(...)` dropped out of
the top-25 table, duplicate hotspots remain `0`, and the largest production functions continue to
measure `60` lines.
LP-CR-1502 isolated workspace summary diagnostics projection into
`_workspace_summary_diagnostics(...)`. `_build_workspace_summary_response(...)` dropped out of the
top-25 table, duplicate hotspots remain `0`, and the largest production functions now measure `59`
lines.
