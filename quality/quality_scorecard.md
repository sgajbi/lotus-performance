...
| Collected tests | 2,035 | 3,224 | 1,189 | measured | `quality/baseline_report.md`; `quality/refactor_health_report.md` |
...
| Largest functions by LOC | unknown | 66 | n/a | measured | `quality/function_size_inventory.md`; `quality/refactor_health_report.md`; LP-CR-1470 moved `_build_flat_period_contribution_result(...)` from `67` to `66` lines and `_build_hierarchy_period_contribution_result(...)` from `64` to `63` lines, so the largest production functions now measure `66` lines |
