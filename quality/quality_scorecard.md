...
| Collected tests | 2,035 | 3,214 | 1,179 | measured | `quality/baseline_report.md`; `quality/refactor_health_report.md` |
...
| Largest functions by LOC | unknown | 70 | n/a | measured | `quality/function_size_inventory.md`; `quality/refactor_health_report.md`; LP-CR-1460 moved `build_mwr_response(...)` out of the top-25 table, so the largest production functions are now `_build_workspace_results_by_period(...)` and `_process_pending_jobs(...)` at `70` lines each |
