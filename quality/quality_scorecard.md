...
| Python LOC | 104,454 | 174,589 | 70,135 | measured | `quality/baseline_report.md`; `quality/refactor_health_report.md` |
...
| Collected tests | 2,035 | 3,431 | 1,396 | measured | `quality/baseline_report.md`; `quality/refactor_health_report.md` |
...
| Average maintainability index | unknown | 55.18 | n/a | measured | `quality/complexity_inventory.md`; `quality/refactor_health_report.md` |
| Largest functions by LOC | unknown | 55 | n/a | measured | `quality/function_size_inventory.md`; `quality/refactor_health_report.md`; `StatefulInputService.get_benchmark_market_series(...)` dropped out of the top-45 table after benchmark market-series retrieval moved behind a typed request boundary, chunk-fetch helper, response projection helper, and snapshot payload builder |
...
| Integration/API/runtime test functions | unknown | 609 | n/a | enforced | `quality/test_taxonomy_inventory.md`; `quality/refactor_health_report.md`; `make quality-test-taxonomy-gate` |
...
| Uncategorized test functions | unknown | 1100 | n/a | enforced ceiling | `quality/test_taxonomy_inventory.md`; `make quality-test-taxonomy-gate`; current ceiling remains `1148` |
...
| Latest architecture signal | 1 | Issue `#344` is fixed as calculation-correctness and reusable platform-pattern hardening: annualization basis resolution now lives in `core.annualize.periods_per_year_for_basis(...)`, Dietz MWR and TWR breakdown consume the same policy, `BUS/252` and explicit `periods_per_year` semantics are API-tested for MWR, largest production functions still measure `55` lines, max CC remains `5`, and test-taxonomy posture remains within the promoted gate with `609` API/runtime tests and `1,100` uncategorized tests. |
