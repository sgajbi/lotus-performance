...
| Collected tests | 2,035 | 3,235 | 1,200 | measured | `quality/baseline_report.md`; `quality/refactor_health_report.md` |
...
| Largest functions by LOC | unknown | 64 | n/a | measured | `quality/function_size_inventory.md`; `quality/refactor_health_report.md`; LP-CR-1479 moved `StatefulInputService._fetch_position_chunk(...)` out of the top-25 table, and the largest production functions are now `resolve_attribution_request(...)` and `LineageMetadataStore._build_pending_payload_stats_statement(...)` at `64` lines |
