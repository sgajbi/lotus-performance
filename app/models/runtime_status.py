...
def _recovery_drill_status_response(status: RecoveryDrillStatus) -> RecoveryDrillStatusResponse:
    return RecoveryDrillStatusResponse(
        status=status.status,
        reason=status.reason,
        active_run_status=status.active_run_status,
        active_run_reason=status.active_run_reason,
        active_run_count=status.active_run_count,
        oldest_active_run_operator_id=status.oldest_active_run_operator_id,
        oldest_active_run_tenant_id=status.oldest_active_run_tenant_id,
        oldest_active_run_governed_target=status.oldest_active_run_governed_target,
        oldest_active_run_acquired_at_utc=status.oldest_active_run_acquired_at_utc,
        oldest_active_run_age_seconds=status.oldest_active_run_age_seconds,
        latest_reclaimed_run_operator_id=status.latest_reclaimed_run_operator_id,
        latest_reclaimed_run_tenant_id=status.latest_reclaimed_run_tenant_id,
        latest_reclaimed_run_governed_target=status.latest_reclaimed_run_governed_target,
        latest_reclaimed_run_acquired_at_utc=status.latest_reclaimed_run_acquired_at_utc,
        latest_reclaimed_run_reclaimed_at_utc=status.latest_reclaimed_run_reclaimed_at_utc,
        latest_reclaimed_run_age_seconds=status.latest_reclaimed_run_age_seconds,
        reclaimed_run_count=status.reclaimed_run_count,
        recent_reclaimed_runs=_operator_action_reclaim_event_responses(status.recent_reclaimed_runs),
        degradation_reasons=list(status.degradation_reasons),
        degradation_details=_degradation_details_response(status.degradation_details),
        latest_generated_at_utc=status.latest_generated_at_utc,
        latest_status=status.latest_status,
        latest_operator_id=status.latest_operator_id,
        latest_backup_identifier=status.latest_backup_identifier,
        latest_age_seconds=status.latest_age_seconds,
    )


def _runtime_retention_status_response(status: RuntimeRetentionStatus) -> RuntimeRetentionStatusResponse:
    return RuntimeRetentionStatusResponse(
        status=status.status,
        reason=status.reason,
        active_run_status=status.active_run_status,
        active_run_reason=status.active_run_reason,
        ...
        preview_reason=status.preview_reason,
        degradation_reasons=list(status.degradation_reasons),
        degradation_details=_degradation_details_response(status.degradation_details),
        latest_generated_at_utc=status.latest_generated_at_utc,
        latest_status=status.latest_status,
        latest_operator_id=status.latest_operator_id,
        latest_trigger_mode=status.latest_trigger_mode,
        latest_job_id=status.latest_job_id,
        latest_cleanup_mode=status.latest_cleanup_mode,
        latest_retention_days=status.latest_retention_days,
        latest_age_seconds=status.latest_age_seconds,
        current_cutoff_utc=status.current_cutoff_utc,
        current_retention_days=status.current_retention_days,
        current_prunable_execution_count=status.current_prunable_execution_count,
        current_prunable_compute_job_count=status.current_prunable_compute_job_count,
        current_prunable_async_result_count=status.current_prunable_async_result_count,
        current_prunable_lineage_record_count=status.current_prunable_lineage_record_count,
        current_prunable_lineage_artifact_count=status.current_prunable_lineage_artifact_count,
    )
...
def build_runtime_status_response(snapshot: RuntimeStatusSnapshot) -> RuntimeStatusResponse:
    return RuntimeStatusResponse(
        ...
        recovery_drill=_recovery_drill_status_response(snapshot.recovery_drill),
        runtime_retention=_runtime_retention_status_response(snapshot.runtime_retention),
