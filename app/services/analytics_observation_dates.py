...
def observation_timestamp_series(values: Iterable[Any]) -> pd.Series:
    index = values.index if isinstance(values, pd.Series) else None
    return pd.to_datetime(pd.Series(values, index=index), utc=True, format="mixed").dt.tz_localize(None)
