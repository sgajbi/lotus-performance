...
- Returns-series product evidence: `POST /integration/returns/series` and
  ...
  use `lotus-reference-market-holidays.v1` rather than a weekday approximation, expose
  `diagnostics.calendar_source`, and fail closed outside the certified 1970-01-01 through
  2099-12-31 horizon. Retained gaps can be bounded through `max_gap_days`,
  `diagnostics.fill_evidence` identifies benchmark or risk-free dates synthesized by
  `FORWARD_FILL` or `ZERO_FILL`, unsupported risk-free day-count conventions are skipped as
  source-quality defects, and large stateless payloads use the same async offload threshold family
  as stateful resolved workloads.
