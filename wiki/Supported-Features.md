...
| Contribution | Portfolio, position, and hierarchy contribution, including stateful source-normalized input | `POST /performance/contribution` | Total, local, and FX contribution results with applied `currency_evidence`, exact EOD FX-coverage refusal, bounded supportability, Carino smoothing evidence, source-economics evidence, trust telemetry, and governed `ContributionAnalytics:v1` data-product declaration. Stateful `BASE_ONLY` preserves Core's portfolio base identity and uses its resolved reporting currency only when every consumed cross-currency position supplies a complete reporting-value pair and every nonzero position-currency cash flow carries the applicable positive Core conversion rates; it refuses incomplete valuations or cash-flow FX rather than relabelling fallback values or inventing FX. That source base/reporting provenance is retained through promoted async execution. RFC-047 proves external-deposit neutrality, income assignment, fee drag, missing classification, short-sleeve sign behavior, downstream Gateway preservation, and Workbench evidence display. Downstream consumers should not reconstruct contribution. |
| Attribution | Portfolio/benchmark attribution, including stateful source-normalized input | `POST /performance/attribution` | Allocation, selection, interaction, active return, currency-attribution and applied-currency evidence, exact EOD FX-coverage refusal, period status, reason codes, residual materiality, bounded source-alignment evidence, supportability metadata, lineage artifacts, governed `AttributionAnalytics:v1` data-product declaration, and merged Gateway/Workbench consumption; fixed-income factor, derivative, sleeve, and composite attribution are not current supported claims. Benchmark-version, classification-version, calendar-policy, and fee/tax/income breakout attribution are also not current supported claims. |
| Composite TWR | Private-banking composite performance from persisted member-return facts | `POST /performance/composites/twr`, `POST /performance/composites/inspect`, `GET /integration/capabilities` surface `composite_twr` | Asset-weighted composite period returns, geometric linking, member weights and contributions, dispersion, blocked/degraded supportability, source fingerprints, source snapshots, restatement versions, classified inspector artifacts, governed `CompositePerformanceAnalytics:v1` data-product declaration, Gateway route realization, Workbench typed BFF consumption, and live RFC-049 proof. Downstream consumers should not reconstruct composite returns, weights, lineage, or restatement posture. Capability publication now advertises composite TWR separately from portfolio TWR so demos and downstream callers do not infer unsupported composite variants. |
| Returns series | Performance-owned return-series bundle for downstream analytics engines | `POST /integration/returns/series`, `make idea-opportunity-runtime-evidence` | Correct downstream surface for risk and Idea opportunity consumers. Returns-series emits coverage, freshness, benchmark-context posture, active-return series, execution identity, and supportability signals. The RFC-0002 Idea proof artifact is source-safe runtime evidence for underperformance and missing-benchmark readiness; it does not promote Idea supported-feature, Gateway, Workbench, Core benchmark-assignment, data-mesh, client-publication, or deployment certification claims. |
...
| Workspace summary | Interaction-efficient performance summary for product surfaces | `POST /performance/workspace-summary` | Product-oriented summary contract for Gateway and Workbench. It consumes performance-owned calculations, does not rebuild them downstream, and binds async success publication to same-calculation lineage metadata plus `lineage_materialization` execution-stage completion so ready results carry ready evidence. |
...
| Runtime operations | Health, readiness, metrics, runtime status, recovery, retention, and build identity | `/health`, `/version`, `/metrics`, `/integration/runtime-status`, recovery and retention routes | Supports enterprise operational posture, CI/runtime diagnostics, and release-audit correlation to image labels, SBOM, vulnerability, and provenance evidence. |
...
| Runtime health, readiness, and build identity | Health, liveness, readiness, metrics, version, runtime status, work-item, recovery, drill, and retention surfaces are implemented. | `/health`, `/health/live`, `/health/ready`, `/version`, `/metrics`, `/integration/runtime-status`, runtime work-item/recovery/retention endpoints | Operations can monitor service posture, correlate a live service to release evidence, and separate application readiness from queued or degraded background work. |
...
| Persisted lineage-volume restart safety | A bounded least-capability initializer repairs shared-volume ownership before non-root API and worker startup; isolated PR/Main proof preserves evidence across restart. | `docker-compose.yml`, `make lineage-volume-recovery-smoke`, [Operations Runbook](Operations-Runbook), [Validation and CI](Validation-and-CI) | Operators can distinguish recoverable mount ownership from data loss and verify restart readiness without deleting client-support evidence. |
...
| Observability and audit middleware | Application startup wires observability and enterprise audit middleware, and static quality gates block missing health/metrics endpoint, correlation propagation, structured logging, metrics, and readiness markers. | `main.py`, `app/observability.py`, `app/enterprise_readiness.py`, `scripts/python_observability_readiness_inventory.py`, `make quality-observability-readiness-gate`, `tests/unit/test_observability.py`, enterprise-readiness tests | Operations and security stakeholders can see that diagnostics and audit posture are part of runtime wiring and protected from accidental regression. |
...
| Validation lane discipline | Fast local and PR-grade gates cover lint, formatting, typecheck, contracts, observability-readiness markers, docs, security, migrations, Docker, tests, and a single demo API certification command. | `make check`, `make quality-observability-readiness-gate`, `make ci`, `make ci-local`, `make demo-api-certification`, Quality Baseline Snapshot workflow, `Makefile`, `README.md`, [Validation and CI](Validation-and-CI) | Delivery and client-project teams can connect feature claims to repeatable release evidence and rerun the demo-critical API/calculation sweep before a presentation. The CI workflow currently publishes demo certification evidence as report-only; it is not yet a blocking readiness gate. |
| Idea opportunity source proof | Source-safe `ReturnsSeriesBundle:v1` evidence for RFC-0002 underperformance and missing-benchmark opportunity archetypes. | `make idea-opportunity-evidence-gate`, `make idea-opportunity-runtime-evidence`, `docs/operations/idea-opportunity-runtime-evidence.md` | Downstream Idea work can clear only Performance-owned source-proof blockers while preserving Core, Gateway, Workbench, data-mesh, client-publication, deployment, and supported-feature blockers until those systems provide their own evidence. |
...
1. Evidence-backed performance numbers
   ...
   Before a demo, run `make demo-api-certification` and review the JSON evidence under
   `output/demo-api-certification/latest.json`. Branch quality snapshots also upload this evidence
   for CI review, but the report-only workflow step should not be treated as a final demo sign-off
